...
def generatePassenger(passengers: list):
    if train[1] == "Up":
        ...
        for i1 in range(g1):
            passengers.append(Passenger(
                len(passengers), -15+TURN_MOVING, 5+i1, "Left", random.randint(1, 5), "Get_off"))
        g2 = random.randint(0, 5)
        for i2 in range(g2):
            passengers.append(Passenger(
                len(passengers), -5+TURN_MOVING, 5+i2, "Left", random.randint(1, 5), "Get_off"))
        g3 = random.randint(0, 5)
        for i3 in range(g3):
            passengers.append(Passenger(
                len(passengers), 5+TURN_MOVING, 5+i3, "Left", random.randint(1, 5), "Get_off"))
        g4 = random.randint(0, 5)
        for i4 in range(g4):
            passengers.append(Passenger(
                len(passengers), 15+TURN_MOVING, 5+i4, "Left", random.randint(1, 5), "Get_off"))
        pass
